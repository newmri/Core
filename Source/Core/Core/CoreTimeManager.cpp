...
TIME_VALUE CoreTimeManager::GetEndSeconds(const TIME_VALUE limitTime) const
{
...
bool CoreTimeManager::IsExpired(const TIME_VALUE time) const
{
	return ((time - GetNowSeconds()) <= 0 ? true : false);
}
